...
#         Mountain Spotted Fever), West Nile, and Dengue - restricted to
#         patients WITHOUT a travel history. Intended as a sensitivity
#         comparison against cosmos_vector_borne (which has no travel-history
#         exclusion) to see how much of the Malaria/Dengue/RMSF signal is
#         travel-associated versus locally acquired.
#
# SlicerDicer session 2853292, "n babesiosis and n malaria and n RMSF and n
# west nile and n dengue and Number of Patients and n lyme by State of
# Residence":
#   Data model      : Patients
#   Population base : All Patients
#   Criteria        : Not Diagnosis (All): Travel abroad,
...
#
# This source previously existed (through the 2026-08-28 export) as a
# NATIONAL-ONLY export with no "State of Residence" stratification, and was
# folded out of the repo on 2026-08-31. The 2026-09-11 export reintroduces it
# with a state breakdown, matching cosmos_vector_borne's layout - so this
# ingest is modeled on that one rather than the old national-only version.
#
# Raw export layout (rows, 1-indexed as in the spreadsheet):
#   1-8   : session metadata
#   11    : measure labels for the value columns (Measures, n babesiosis,
#           n malaria, n RMSF, n west nile, n dengue, Number of Patients,
#           n lyme)
...
#
# Conventions applied here (see cosmos_vector_borne for the sibling source
# with no travel-history exclusion):
...
#     population base (not travel history, has any encounters), not a rate
#     per 100,000, and NOT directly comparable to cosmos_vector_borne's
#     percentages because the denominator criteria differ - compare the
#     epic_n_* counts across the two sources instead
# =============================================================================
