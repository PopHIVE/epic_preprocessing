...
raw <- dcf::dcf_process_epic_staging(cleanup=T, standard_names = c(
  opioid_od="OPIOID OD",
  heat_related ="Heat",
...
chronic_import <- function(yearset){
  a1 <- read_csv(paste0("./raw/staging_chronic/2025_11_12_C02_DM_A1C_State_Age_",yearset,".csv"), skip = 11) %>%
...
all_chronic_county <- all_obesity_county %>% 
  full_join(all_diabetes_county, by=c('geography', 'time', 'age','n_patients_chronic')
  )
